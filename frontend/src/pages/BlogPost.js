...
const MarkdownComponents = {
  h2: ({ children }) => {
    ...
    return <h2 id={id} className="text-2xl font-bold text-slate-800 mt-8 mb-4 scroll-mt-20">{children}</h2>;
  },
  h3: ({ children }) => {
    const id = children.toString().toLowerCase().replace(/[^a-z0-9]+/g, '-');
    return <h3 id={id} className="text-xl font-semibold text-slate-800 mt-6 mb-3 scroll-mt-20">{children}</h3>;
  },
  p: ({ children }) => <p className="text-slate-600 leading-relaxed mb-4">{children}</p>,
  ul: ({ children }) => <ul className="list-disc list-inside mb-4 space-y-2 text-slate-600">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal list-inside mb-4 space-y-2 text-slate-600">{children}</ol>,
  li: ({ children }) => <li className="leading-relaxed">{children}</li>,
  a: ({ href, children }) => (
    <a href={href} className="text-green-600 hover:text-green-700 underline" target={href?.startsWith('http') ? '_blank' : undefined} rel={href?.startsWith('http') ? 'noopener noreferrer' : undefined}>
      {children}
    </a>
  ),
  blockquote: ({ children }) => (
    <blockquote className="border-l-4 border-green-500 pl-4 italic text-slate-600 my-4 bg-green-50 py-3 pr-4 rounded-r">
      {children}
    </blockquote>
  ),
  ...
  img: ({ src, alt }) => (
    <img 
      src={src?.startsWith('/') ? `${BACKEND_URL}${src}` : src} 
      alt={alt} 
  ...
  th: ({ children }) => <th className="bg-slate-100 px-4 py-2 text-left font-semibold text-slate-700 border-b">{children}</th>,
