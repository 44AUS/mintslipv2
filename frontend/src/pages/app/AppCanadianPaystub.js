...
// midnight, which reads back as the previous day in North American timezones.
...
export default function AppCanadianPaystub() {
  ...
  const handleSubscriptionDownload = async () => {
    ...
    try {
      ...
      showToast("Canadian pay stub(s) downloaded successfully!", "success");
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const { ok, data } = await nativePost(`${BACKEND_URL}/api/validate-coupon`, { code, generatorType: "canadian-paystub" });
        if (cancelled) return;
        if (ok && data?.valid) {
  ...
  const handlePaymentSuccess = ({ email, paymentIntentId }) => {
    ...
    localStorage.setItem("pendingResetKeys", JSON.stringify([STORAGE_KEY, "canadianPaystubTemplate", "canadianPaystubCompanyLogo"]));
        ...
        <div style={{ background: "var(--ion-card-background)", borderRadius: 6, padding: "20px 20px 24px", height: "100%", overflowY: "auto", boxShadow: "0 2px 12px rgba(0,0,0,0.10)", boxSizing: "border-box" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {templateCards.map((company, cardIdx) => (
              <div key={company.id} className="app-sample-card ion-activatable"
        ...
        <div className="modal-backdrop" style={{ position: "fixed", inset: 0, zIndex: 10001, background: window.innerWidth >= 768 ? "rgba(0,0,0,0.5)" : "var(--ion-background-color, #f2f2f7)", display: "flex", alignItems: window.innerWidth >= 768 ? "center" : "stretch", justifyContent: window.innerWidth >= 768 ? "center" : "stretch" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
              {isGeneratingPreview ? (
                ...
                <>
                  <PreviewPager pages={pdfPreviews} index={previewPageIndex} onIndexChange={setPreviewPageIndex} altPrefix="Canadian pay stub preview" />
