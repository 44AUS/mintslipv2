...
export default function AppOfferLetter() {
  ...
  // Persist to localStorage (include images)
  useEffect(() => {
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      const { ok, data } = await nativePost(`${BACKEND_URL}/api/stripe/create-one-time-checkout`, {
        ...
        cancelUrl:    `${origin}/app/offer-letter`,
      });
  ...
  const Field = ({ label, field, type = "text", placeholder = "" }) => (
    <div>
      ...
      <IonInput
        ...
        onIonInput={e => setField(field, e.detail.value)}
        type={type}
  ...
  const TemplateBtn = ({ t }) => (
    ...
    >
      <div style={{ fontSize: "0.8rem", fontWeight: 700, color: formData.template === t.value ? "var(--ion-color-primary)" : "var(--ion-text-color)" }}>
        {t.label}
      </div>
      ...
      {formData[typeField] === "custom" && (
        formData[imageField] ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 8, border: "1px solid var(--ion-color-step-200)", background: "var(--ion-color-step-50)" }}>
            ...
            <span style={{ flex: 1, fontSize: "0.78rem", color: "var(--ion-color-medium)" }}>Signature uploaded</span>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "16px 16px 120px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {TEMPLATES.map(t => <TemplateBtn key={t.value} t={t} />)}
          </div>
