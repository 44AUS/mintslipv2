...
export default function AppPaystub() {
  ...
  const isKnownTemplate = (v) =>
    ["template-a","template-b","template-c","template-h"].includes(v) || (typeof v === "string" && v.startsWith("custom:"));
  ...
  useEffect(() => {
    Promise.all([
      ...
      generateAllPreviewImages(PREVIEW_SAMPLE_DATA, "template-h", 1),
    ]).then(([a, c, h]) => {
  ...
  useEffect(() => {
    customTemplates.forEach((t) => {
      ...
      generateAllPreviewImages(PREVIEW_SAMPLE_DATA, key, 1)
    ...
    const timer = setTimeout(async () => {
      if (formData.startDate && formData.endDate && (formData.rate || formData.annualSalary)) {
        ...
        try {
          ...
          const previews = await generateAllPreviewImages(previewData, selectedTemplate, numStubs);
  ...
  const handleNext = async () => {
    ...
    if (pdfPreviews.length === 0) {
      ...
      try {
        ...
        const previews = await generateAllPreviewImages(previewData, selectedTemplate, calculateNumStubs);
        setPdfPreviews(previews);
        setPreviewPageIndex(0);
      } catch (err) { console.error("Preview generation failed:", err); }
      setIsGeneratingPreview(false);
        ...
        <div style={{ background: "var(--ion-card-background)", borderRadius: 6, padding: "20px 20px 24px", height: "100%", overflowY: "auto", boxShadow: "0 2px 12px rgba(0,0,0,0.10)", boxSizing: "border-box" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {templateCards.map((company, cardIdx) => (
              <div key={company.id} className="app-sample-card"
      ...
      {formModalOpen && createPortal(
        <div className="modal-backdrop" style={{ position: "fixed", inset: 0, zIndex: 10000, background: window.innerWidth >= 768 ? "rgba(0,0,0,0.5)" : "var(--ion-background-color, #f2f2f7)", display: "flex", alignItems: window.innerWidth >= 768 ? "center" : "stretch", justifyContent: window.innerWidth >= 768 ? "center" : "stretch" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 40px" }}>
            ...
            <div style={{ marginTop: 12 }}>
              <IonButton expand="block" onClick={handleNext} disabled={isGeneratingPreview}
                style={{ "--background": "#059669", "--background-activated": "#047857" }}>
                {isGeneratingPreview ? <IonSpinner name="crescent" style={{ color: "#fff" }} /> : "Preview"}
              </IonButton>
