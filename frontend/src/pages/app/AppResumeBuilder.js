...
export default function AppResumeBuilder() {
  ...
  const [generatedResume, setGeneratedResume] = useState(() => {
    try {
      const s = localStorage.getItem(GENERATED_KEY);
  ...
  const [isGeneratingResponsibilities, setIsGeneratingResponsibilities] = useState(null);
  ...
  useEffect(() => {
    try {
      if (generatedResume) localStorage.setItem(GENERATED_KEY, JSON.stringify(generatedResume));
    } catch {}
  }, [generatedResume]);
  ...
  // Debounced preview whenever generated resume or style changes
  useEffect(() => {
  ...
  const triggerPreview = async (resume) => {
    ...
    try {
      const url = await generateResumePreview({
        ...resume,
  ...
  const addWork = () =>
    setFormData(prev => ({ ...prev, workExperience: [...prev.workExperience, makeWorkEntry()] }));

  const removeWork = id =>
    setFormData(prev => ({ ...prev, workExperience: prev.workExperience.filter(e => e.id !== id) }));

  const addBullet = expId =>
    setFormData(prev => ({
      ...prev,
      workExperience: prev.workExperience.map(e =>
        e.id === expId ? { ...e, responsibilities: [...e.responsibilities, ""] } : e
      ),
    }));
  ...
  const generateAIBullets = async expId => {
    ...
    if (!exp?.position || !exp?.company) {
      showToast("Enter job title and company first", "warning");
      return;
    }
    setIsGeneratingResponsibilities(expId);
    try {
      ...
      if (data.success && data.responsibilities) {
        setFormData(prev => ({
          ...prev,
          workExperience: prev.workExperience.map(e =>
            e.id === expId ? { ...e, responsibilities: data.responsibilities } : e
          ),
        }));
      ...
      showToast(err.message || "Failed to generate bullets", "danger");
    } finally {
      setIsGeneratingResponsibilities(null);
    }
  };
  ...
  const addEdu = () =>
    setFormData(prev => ({ ...prev, education: [...prev.education, makeEduEntry()] }));
  ...
  const updateSkill = (idx, val) =>
    setFormData(prev => ({ ...prev, skills: prev.skills.map((s, i) => i === idx ? val : s) }));
  ...
  const generateResume = async () => {
    if (!formData.jobDescription) {
      showToast("Please provide a job description in Step 5", "warning");
      return;
    }
    if (!formData.personalInfo.fullName) {
      showToast("Please fill in your name in Step 1", "warning");
      return;
    }
    ...
    try {
      ...
      showToast(err.message || "Failed to generate resume. Please try again.", "danger");
    } finally {
  ...
  const scrapeJobUrl = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/scrape-job`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      const { ok, data } = await nativePost(`${BACKEND_URL}/api/stripe/create-one-time-checkout`, {
        ...
        cancelUrl:  `${origin}/app/resume-builder`,
      });
  ...
  const Field = ({ label, field, subfield, type = "text", placeholder = "", value, onChange }) => {
    const val = value !== undefined ? value : (subfield ? formData[field][subfield] : formData[field]);
    const handleChange = onChange || (v => {
      if (subfield) {
        setFormData(prev => ({ ...prev, [field]: { ...prev[field], [subfield]: v } }));
      } else {
        setField(field, v);
      }
    });
    return (
      <div>
        ...
        <IonInput value={val} onIonInput={e => handleChange(e.detail.value)}
  ...
  const renderStep1 = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Field label="Full Name *" field="personalInfo" subfield="fullName" />
      <Field label="Email" field="personalInfo" subfield="email" type="email" />
      <Field label="Phone" field="personalInfo" subfield="phone" type="tel" />
      <Field label="Location (City, State)" field="personalInfo" subfield="location" />
      <Field label="LinkedIn URL" field="personalInfo" subfield="linkedin" />
      <Field label="Portfolio / Website" field="personalInfo" subfield="website" />
    </div>
  );
        ...
        <div key={exp.id} style={{ padding: 14, borderRadius: 8, border: "1px solid var(--ion-color-step-200)", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <IonButton fill="clear" size="small" onClick={() => generateAIBullets(exp.id)} disabled={isGeneratingResponsibilities === exp.id}
                style={{ "--color": "#7c3aed", fontSize: "0.72rem", height: 28 }}>
                {isGeneratingResponsibilities === exp.id
            ...
            {exp.responsibilities.map((r, rIdx) => (
              <div key={rIdx} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                <IonInput value={r} onIonInput={e => updateBullet(exp.id, rIdx, e.detail.value)}
                  placeholder={`Bullet ${rIdx + 1}`} fill="outline" style={{ ...inputStyle, flex: 1 }} />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {formData.education.map((edu, idx) => (
        <div key={edu.id} style={{ padding: 14, borderRadius: 8, border: "1px solid var(--ion-color-step-200)", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Field label="Degree" value={edu.degree} onChange={v => updateEdu(edu.id, "degree", v)} placeholder="B.S., M.A., etc." />
  ...
  const renderStep4 = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <p style={{ fontSize: "0.82rem", color: "var(--ion-color-medium)", margin: 0 }}>
        Add technical skills, tools, languages, certifications, etc.
      </p>
      {formData.skills.map((skill, idx) => (
        <div key={idx} style={{ display: "flex", gap: 6 }}>
          <IonInput value={skill} onIonInput={e => updateSkill(idx, e.detail.value)}
            placeholder={`Skill ${idx + 1}`} fill="outline" style={{ ...inputStyle, flex: 1 }} />
  ...
  const renderStep5 = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Field label="Target Job Title" field="targetJobTitle" placeholder="e.g. Senior Software Engineer" />

      {/* URL scraper */}
      <div>
        <span style={labelStyle}>Job Posting URL (Optional)</span>
        <div style={{ display: "flex", gap: 8 }}>
          <IonInput value={formData.jobUrl} onIonInput={e => setField("jobUrl", e.detail.value)}
            placeholder="Paste job URL to auto-extract description" fill="outline" style={{ ...inputStyle, flex: 1 }} />
      ...
      <div>
        <span style={labelStyle}>Job Description * (paste or extract above)</span>
        <IonTextarea value={formData.jobDescription} onIonInput={e => setField("jobDescription", e.detail.value)}
          rows={10} placeholder="Paste the full job description here. The AI uses this to tailor your resume." fill="outline"
          ...
          {TEMPLATES.map(t => (
            <button key={t.value} onClick={() => setField("template", t.value)}
              style={{
                ...
                border: `2px solid ${formData.template === t.value ? t.color : "var(--ion-color-step-200)"}`,
                background: formData.template === t.value ? `${t.color}15` : "transparent",
                cursor: "pointer", textAlign: "center",
              }}>
              <div style={{ fontSize: "0.78rem", fontWeight: 700, color: formData.template === t.value ? t.color : "var(--ion-text-color)" }}>
                {t.label}
              </div>
      ...
      {generatedResume && (
        <>
          <div style={{ padding: 14, borderRadius: 8, background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.3)", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: "0.85rem", color: "var(--ion-text-color)", fontWeight: 500 }}>
              Resume generated — ready to download
            </span>
            <IonButton fill="clear" size="small" onClick={generateResume}
              style={{ "--color": "var(--ion-color-medium)", marginLeft: "auto", flexShrink: 0 }}>
  ...
  return (
    <AppLayout>
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "16px 16px 120px", display: "flex", flexDirection: "column", gap: 16 }}>

        {/* Step tabs */}
        <IonSegment
          scrollable
          value={String(currentStep)}
          onIonChange={e => setCurrentStep(Number(e.detail.value))}
          style={{ "--background": "var(--ion-card-background)", borderRadius: 8, boxShadow: "rgba(0,0,0,0.10) 0px 2px 8px" }}
        >
          {STEPS.map(step => (
            <IonSegmentButton
              key={step.id}
            ...
            >
              <IonIcon icon={step.icon} style={{ fontSize: 18 }} />
              <IonLabel style={{ fontSize: "0.65rem", fontWeight: 600 }}>{step.label}</IonLabel>
            </IonSegmentButton>
          ))}
        </IonSegment>
