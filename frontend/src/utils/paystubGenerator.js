...
async function generateSingleStub(
  ...
  consistentHireDate = null  // Pass consistent hire date from parent function
) {
  const payType = formData.payType || "hourly";
  const workerType = formData.workerType || "employee";
  const isContractor = workerType === "contractor";
  ...
  // Contractors don't have taxes withheld - taxes calculated on taxableIncome (includes cash tips)
