...
export function generateTemplateH(doc, data, pageWidth, pageHeight, margin) {
  ...
  // Labels
  doc.text("Check #", m + 5, y + 24);
  doc.text("Check Date", m + 55, y + 24);
  ...
  doc.setFontSize(8);
  doc.setTextColor(...colors.white);
  doc.text("Withholding Taxes", col2X + colWidth/2, y + 9, { align: 'center' });
  ...
  const taxData = [
    ...
    ["Add MEDI", "", ""],
    ["SDI WTH-" + (formData.state || "ST"), "", ""],
    ["SIT-" + (formData.state || "ST"), fmtCurrency(stateTax), fmtCurrency(ytdStateTax)],
    ["", "", ""],
    ["", "", ""],
    ["", "", ""],
    ["", "", ""],
  ...
  doc.text(fmtCurrency(ytdSsTax + ytdMedTax + ytdFederalTax + ytdStateTax), col2X + colWidth - 5, y + 8, { align: 'right' });
  ...
  // Accruals column
  const accrualsX = m + 5;
  doc.text("Sick/PTO:", accrualsX, contentY + 5);
  ...
  doc.text(`${formData.companyCity || "City"}, ${formData.companyState || "ST"}`, companyX, contentY + 25);
  doc.text(formData.companyZip || "00000", companyX, contentY + 35);
